...
class CreateMissingPersonSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateMissingPersonSheet> createState() => _CreateMissingPersonSheetState();
}

class _CreateMissingPersonSheetState extends ConsumerState<CreateMissingPersonSheet> {
      ...
      if (faces.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please upload a clear photo of the missing person\'s face.')),
          );
        }
  ...
  Future<void> _submit() async {
    ...
    final post = FeedPost(
      ...
      mpName: _nameController.text.trim(),
      mpSurname: _surnameController.text.trim(),
      mpAge: int.tryParse(_ageController.text),
      mpLastSeen: _lastSeenController.text.trim(),
      mpContactName: _contactNameController.text.trim(),
      mpContactPhone: _contactPhoneController.text.trim(),
    );
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  TextButton(
                    ...
                    child: _isPosting 
                        ...
                        : const Text('Post', style: TextStyle(color: AppColors.orange, fontWeight: FontWeight.bold)),
                  ),
                ],
              ),
                    ...
                    child: _isValidating
                        ...
                        : _image == null
                            ? const Column(
                                ...
                                children: [Icon(Icons.camera_alt, color: Colors.grey), Text('Upload Photo', style: TextStyle(fontSize: 10, color: Colors.grey))],
                              )
              ...
              const SizedBox(height: 12),
              _buildTextField(_lastSeenController, 'Last Seen (Date & Location)'),
              const SizedBox(height: 12),
              _buildTextField(_contactNameController, 'Contact Name'),
              const SizedBox(height: 12),
              _buildTextField(_contactPhoneController, 'Contact Phone', keyboardType: TextInputType.phone),
              const SizedBox(height: 12),
